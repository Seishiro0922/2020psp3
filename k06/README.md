...
74~78  　表（nap_value[items][nap_size])の初期化

80 　　 商品⼀つずつについて、所持金の少ないところから順番に、処理を実行する

81~83  表の所持金の値が商品の値段より小さい場合、表の1つ上の値をコピー

　　　　（所持金の方が小さい場合、最大カロリーは変化しない）

85～92　商品の値段までは上の値がコピーされているので、値段の位置から処理を始める

　　　　入れ替えない場合のカロリーと入れ替えた場合のカロリーを比較して、大きいほうを表に代入する
